# app_railway.py - Railway deployment optimized version
...
def ready_check():
    ...
    try:
        if not data_loaded:
            load_data()
        return jsonify({'status': 'ready', 'message': 'Data loaded successfully'})
    except Exception as e:
        return jsonify({'status': 'loading', 'message': str(e)}), 503
